...
def decode_powershell(text):
  ...
  for line in text.split('\n'):
    ...
    if re.search(r'(-en?c?o?d?e?d?C?o?m?m?a?n?d? |\[(System\.)*Text\.Encoding\]::\w{4,}\.GetString\(\[(System\.)*Convert\]::FromBase64String\(|\[(System\.)*Convert\]::FromBase64String\(\[Text\.Encoding\]::\w{4,}\.GetBytes\()(\s*)(\S+)', line):
      # Find the base64 encoded string
      encoded_str1 = re.search(r'(-en?c?o?d?e?d?C?o?m?m?a?n?d? |\[(System\.)*Text\.Encoding\]::\w{4,}\.GetString\(\[(System\.)*Convert\]::FromBase64String\(|\[(System\.)*Convert\]::FromBase64String\(\[Text\.Encoding\]::\w{4,}\.GetBytes\()(\s*)(\S+)', line).group(0)
      #print("this is the encoded changes " + str(encoded_str1))
      encoded_str = re.search(r'(-en?c?o?d?e?d?C?o?m?m?a?n?d? |\[(System\.)*Text\.Encoding\]::\w{4,}\.GetString\(\[(System\.)*Convert\]::FromBase64String\(|\[(System\.)*Convert\]::FromBase64String\(\[Text\.Encoding\]::\w{4,}\.GetBytes\()(\s*)(\S+)', line).group(6)
      ...
      if encoded_str:
        ...
        lline = line.replace(encoded_str1, decoded_str)
