...
def create_file_with_zeros(size_in_kb, file_format):
    ...
    with open(f"all_zeros.{file_format}", "wb") as file:
        ...
        file.write(b'\0' * (size_in_bytes - file.tell()))
...
file_format = input("Enter the desired file format (png, jpg, mp3, wav, docx, xlsx, pptx, zip, 7z, rar, txt, csv, exe, elf, pdf, iso, img, All): ").lower()

# Create files of all formats if "All" is selected
if file_format == "all":
    for format in ["png", "jpg", "mp3", "wav", "docx", "xlsx", "pptx", "zip", "7z", "rar", "txt", "csv", "exe", "elf", "pdf", "iso", "img"]:
        create_file_with_zeros(size_in_kb, format)
        print(f"File 'all_zeros.{format}' created with {size_in_kb} KB of zeros and {format} format.")
else:
    # Create the file with all zeros and magic numbers
    create_file_with_zeros(size_in_kb, file_format)
    print(f"File 'all_zeros.{file_format}' created with {size_in_kb} KB of zeros and {file_format} format.")
