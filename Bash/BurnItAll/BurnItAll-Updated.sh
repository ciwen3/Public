...
if [[ ! -f SHA256-Check.txt ]]; then touch SHA256-Check.txt; fi

# check if folder exists if not create it
if [[ ! -d $(date +"%d-%b-%Y") ]]; then mkdir $(date +"%d-%b-%Y"); else mv $(date +"%d-%b-%Y") $(date +"%d-%b-%Y")-old; mkdir $(date +"%d-%b-%Y"); fi
cd $(date +"%d-%b-%Y")

# add blank space and then the date to the file. 
